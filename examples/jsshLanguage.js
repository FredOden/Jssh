...
var rules = {
	...
	,atom : {
		...
		, "Javascript" : (p) => () => Activity.loadScript(p.$(0)().slice(1, -1), "stdin").s
		, "π" : (p) => () => Math.PI
		, "String": p => () => p.$(0)().slice(1, -1)
		, "&variable":
		(p) => () => {
			/*
    ...
    */
		...
		, "print &expr":
		(p) => () => { java.lang.System.out.println(p.$(1)());return p.$(1)(); }
		, "if &factor then &expr":
		p => () => {
			if ($N(p.$(1)())) return p.$(3)();
...
const jsshLanguageParser = new Lourah.utils.text.Parser(lexicon, rules, "statement");
